...
// ── Master chain: Saturator → Stereo Widener → Compressor → Limiter → Reverb (room glue) ──
const masterReverb = new Tone.Reverb({ decay: 2.2, wet: 0.08 }).toDestination();
const masterLimiter = new Tone.Limiter(-0.5).connect(masterReverb);
...
// Stereo widener: slight mid/side separation via chorus at inaudible depth
const masterWidener = new Tone.Chorus({
  frequency: 0.4,
  depth: 0.12,
  wet: 0.08,
  spread: 180,
}).connect(masterCompressor).start();
// Tape-style saturation via waveshaper
const masterSaturator = new Tone.Chebyshev(2).connect(masterWidener);
masterSaturator.wet.value = 0.06;
...
const INSTRUMENT_DEFS: Record<InstrumentKey, InstrumentDef> = {
  ...
  // Mark I Rhodes — bell-like FM tines with warm body
  rhodes: {
    create: () => new Tone.PolySynth(Tone.FMSynth, {
      harmonicity: 3.5,
      modulationIndex: 2.2,
      oscillator: { type: 'sine' },
      envelope: { attack: 0.002, decay: 0.9, sustain: 0.15, release: 2.2 },
  ...
  // Analog-style sub bass — round sine with fast attack
  analog_bass: {
    create: () => new Tone.PolySynth(Tone.Synth, {
      oscillator: { type: 'sine4' },
      envelope: { attack: 0.005, decay: 0.35, sustain: 0.6, release: 0.3 },
      volume: -6,
    }),
  },
  upright_bass: {
  ...
  // Celeste — crystalline FM with high harmonicity
  celeste: {
    create: () => new Tone.PolySynth(Tone.FMSynth, {
      harmonicity: 12,
      modulationIndex: 0.3,
      ...
      modulation: { type: 'sine' },
      modulationEnvelope: { attack: 0.001, decay: 1, sustain: 0, release: 2 },
      volume: -16,
    }),
  },
  pads: {
  ...
  // Warm analog-style pad — detuned saws with slow attack
  warm_pad: {
    create: () => new Tone.PolySynth(Tone.Synth, {
  ...
  // Pluck synth — fast attack, quick decay, perfect for arpeggios
  pluck: {
    create: () => new Tone.PolySynth(Tone.FMSynth, {
      harmonicity: 4,
      modulationIndex: 1.8,
  ...
  // Muted trumpet — nasal FM for jazz counter-lines
  muted_trumpet: {
    create: () => new Tone.PolySynth(Tone.FMSynth, {
      harmonicity: 2,
      modulationIndex: 3,
  ...
  // Soft brass section — warm ensemble stab
  soft_brass: {
    create: () => new Tone.PolySynth(Tone.Synth, {
...
function createTrackEffects(track: Track): Tone.ToneAudioNode[] {
  ...
  for (const effect of track.effects) {
    switch (effect.type) {
      ...
      case 'widen': {
        const widen = new Tone.Chorus({
          frequency: 0.3,
          depth: effect.params.depth ?? 0.15,
          wet: effect.wet,
          spread: 180,
        }).start();
        fxChain.push(widen);
      ...
      case 'saturator': {
        const sat = new Tone.Chebyshev(effect.params.order ?? 2);
        sat.wet.value = effect.wet;
        fxChain.push(sat);
        break;
      }
    }
  ...
  if (track.name === 'Drums') {
    const drumSynths = {
      kick: new Tone.MembraneSynth({
        ...
        octaves: 6,
        volume: -6,
        envelope: { attack: 0.001, decay: 0.25, sustain: 0.01, release: 0.15 },
      }),
      ...
      // Rimshot / cross-stick — tighter noise burst
    ...
    drumSynthInstances.push(drumSynths.kick, drumSynths.snare, drumSynths.hihat, drumSynths.rim);
    ...
    connectDrumSynthToChain(drumSynths.rim, fxChain, channel);
    ...
    for (const note of track.notes) {
      ...
      if (note.pitch === 36) {
        ...
        // rim click
        transport.schedule((t) => {
          drumSynths.rim.triggerAttackRelease('16n', t, vel * 0.6);
        }, time);

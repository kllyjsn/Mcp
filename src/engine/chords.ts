...
function applyChordEnrichment(
  ...
  if (complexity >= 6 && (tension > 0.3) && Math.random() > 0.6) {
...
export function generateChordProgression(
  ...
  for (let measure = 0; measure < measures; measure++) {
    ...
    let rootIndex = (NOTE_NAMES.indexOf(resolveNoteName(key)) + rootInterval) % 12;
    ...
    if (complexity >= 7 && tension > 0.6 && diatonic.quality === 'dominant7' && Math.random() > 0.6) {
      ...
      const secDomRoot = (NOTE_NAMES.indexOf(resolveNoteName(key)) + secDomInterval + 7) % 12;
      if (Math.random() > 0.5) {
      ...
      if (parallelIntervals && degree < parallelIntervals.length) {
        ...
        rootIndex = (NOTE_NAMES.indexOf(resolveNoteName(key)) + borrowedInterval) % 12;

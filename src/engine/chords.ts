...
export function generateChordProgression(
  ...
  for (let measure = 0; measure < measures; measure++) {
    ...
    if (complexity >= 6 && Math.random() > 0.5) {
    ...
    if (complexity >= 8 && Math.random() > 0.5) {
    ...
    if (complexity >= 9 && degree === 4 && Math.random() > 0.6) {
      quality = 'altered';
    }
    if (complexity >= 7 && style === 'jazz' && Math.random() > 0.75) {
      if (quality === 'major7') quality = 'sixth';
      else if (quality === 'minor7') quality = 'minor6';
    }
